...
export default function Stocks() {
  ...
  const loadAllData = () => {
    ...
    Object.entries(parsedGoodsReturned).forEach(([orderId, entries]) => {
      if (entries && entries.length > 0) {
        ...
        if (matchingOrder) {
          // Get latest entry date for sorting
          ...
          entries.forEach((entry) => {
            ...
            if (!productsMap[productKey]) {
              // Find original product in matchingOrder to get order quantity
              const originalProduct = matchingOrder.products?.find(
              ...
              productsMap[productKey] = {
                ...
                totalReceived: 0,
                orderQuantity: originalProduct?.quantity || 0,
                entries: [],
              };
            }
          ...
          ordersWithGoods.push({
            ...
            latestEntryDate: latestEntryDate, // Add for sorting
          });
        }
      }
    });
    ...
    const sortedOrders = ordersWithGoods.sort((a, b) => {
      const dateA = a.latestEntryDate;
      const dateB = b.latestEntryDate;
  ...
  const getVerifiedQuantity = (productId, orderId) => {
    const verifiedData = stocksVerifiedData[orderId] || [];
    const verified = verifiedData.find((v) => v.productId === productId);
    return verified?.quantityVerified || 0;
  };
