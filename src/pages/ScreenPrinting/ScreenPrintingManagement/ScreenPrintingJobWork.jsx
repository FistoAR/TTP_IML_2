...
export default function ScreenPrintingJobWork() {
  ...
  const hasJobWorkEntries = (productId, order) => {
    const { productTotals: jobWorkTotals } = calculateJobWorkTotals(order.id);
    return (jobWorkTotals[productId] || 0) > 0;
  };
  ...
  useEffect(() => {
   ...
   useEffect(() => {
      if (location.state?.activeSheet) {
        setViewMode(location.state.activeSheet);
        // Clear the state
        window.history.replaceState({}, document.title);
      }
    }, [location.state]);

  // Calculate job work totals for a specific order
  ...
  // Calculate goods returned totals for a specific order
  const calculateGoodsReturnedTotals = (orderId) => {
    try {
      ...
      if (stored) {
        ...
        orderEntries.forEach((entry) => {
          ...
          productTotals[entry.productId] += entry.quantityReceived;
        });

        return { totalEntries: orderEntries.length, productTotals };
      }
    } catch (error) {
      console.error("Error calculating goods returned totals:", error);
    }
    return { totalEntries: 0, productTotals: {} };
  };

  // Filter orders for goods returned view - only show orders that have job work entries
  const filterOrdersForGoodsReturned = (orders) => {
    try {
      const storedJobWork = localStorage.getItem(JOBWORK_STORAGE_KEY);
      if (!storedJobWork) return [];

      const allJobWorkEntries = JSON.parse(storedJobWork);

      return orders.filter((order) => {
        // Check if this order has any job work entries
        const orderJobWorkEntries = allJobWorkEntries[order.id] || [];
        return orderJobWorkEntries.length > 0;
      });
    } catch (error) {
      console.error("Error filtering orders for goods returned:", error);
      return orders;
    }
  };
  ...
  const groupOrdersByCompany = () => {
    let filteredOrders = [...screenPrintingOrders];

    // If in goods returned view, filter out orders without job work
    if (viewMode === "goodsreturned") {
      filteredOrders = filterOrdersForGoodsReturned(filteredOrders);
    }
  ...
  // Handle edit with dynamic routing based on viewMode
  ...
  // Calculate remaining quantity for job work
  ...
  // Calculate received and pending for goods returned
  const calculateReceivedAndPending = (productId, order) => {
    const orderQuantity = parseInt(
      order.products?.find((p) => p.id === productId)?.quantity || 0,
    );
    const { productTotals: jobWorkTotals } = calculateJobWorkTotals(order.id);
    const { productTotals: returnedTotals } = calculateGoodsReturnedTotals(
      order.id,
    );

    const sentQuantity = jobWorkTotals[productId] || 0;
    const receivedQuantity = returnedTotals[productId] || 0;
    const pendingQuantity = Math.max(0, sentQuantity - receivedQuantity);

    return { orderQuantity, receivedQuantity, pendingQuantity };
  };
  ...
  const getPageSubtitle = () => {
    if (viewMode === "jobwork") {
      return "Manage job work entries for screen printing orders";
    } else if (viewMode === "goodsreturned") {
      return "Track goods returned from job work (Only shows orders with job work entries)";
    }
    return "";
  };
      ...
      <div className="mb-[1vw]">
        <div className="flex justify-between items-center mb-[0.25vw]">
          <div>
            ...
            <p className="text-[0.9vw] text-gray-600 mt-1">
              {getPageSubtitle()}
            </p>
          </div>
      ...
      {!hasOrders ? (
        <div className="bg-white rounded-xl shadow-sm p-[2vw] text-center border border-gray-200">
          ...
          <p className="text-gray-600 text-[0.9vw]">
            {searchTerm || selectedProduct || selectedSize
              ...
              : viewMode === "goodsreturned"
                ? "No orders have job work entries yet. Add job work entries first to track goods returned."
        ...
        <div className="space-y-[1.5vw] max-h-[60vh] overflow-y-auto">
          ...
          {Object.entries(filteredGroupedOrders).map(
            ([companyName, orders]) => {
              const jobWorkTotalsForCompany = Object.values(orders).reduce(
                (acc, order) => {
                  const { totalEntries } = calculateJobWorkTotals(order.id);
                  return acc + totalEntries;
                },
                0,
              );

              const returnedTotalsForCompany = Object.values(orders).reduce(
                (acc, order) => {
                  const { totalEntries } = calculateGoodsReturnedTotals(
                    order.id,
                  );
                  return acc + totalEntries;
                },
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    className={`text-white px-[1.5vw] py-[0.85vw] cursor-pointer  transition-all flex justify-between items-center 
                    ${viewMode === "jobwork" ? "bg-[#3d64bb] hover:bg-[#2f4d99] " : "bg-green-600 hover:bg-green-700"}`}
                  >
                    ...
                    <div className="text-[0.85vw] bg-white/20 px-4 py-2 rounded-full">
                      {viewMode === "jobwork"
                        ? `Job Work Entries: ${jobWorkTotalsForCompany}`
                        : `Goods Returned Entries: ${returnedTotalsForCompany}`}
                    </div>
                  </div>
                  ...
                  {expandedCompanies[companyName] && (
                    <div className="space-y-[1.25vw] p-[1vw]">
                      {Object.entries(orders).map(([orderKey, order]) => {
                        ...
                        const { totalEntries: jobWorkEntriesCount } =
                          calculateJobWorkTotals(order.id);
                        const { totalEntries: returnedEntriesCount } =
                          calculateGoodsReturnedTotals(order.id);
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="bg-gray-200 px-[1.5vw] py-[0.85vw] transition-all flex justify-between items-center">
                                ...
                                <div className="flex-1">
                                  <div className="flex items-center gap-4 flex-wrap">
                                    ...
                                    <div className="flex gap-3">
                                      {viewMode === "jobwork" &&
                                        jobWorkEntriesCount > 0 && (
                                          <span className="bg-blue-100 text-blue-800 text-[0.8vw] px-2 py-1 rounded-full">
                                            {jobWorkEntriesCount} Job Work
                                            Entries
                                          </span>
                                        )}
                                      {viewMode === "goodsreturned" &&
                                        returnedEntriesCount > 0 && (
                                          <span className="bg-green-100 text-green-800 text-[0.8vw] px-2 py-1 rounded-full">
                                            {returnedEntriesCount} Returned
                                            Entries
                                          </span>
                                        )}
                                    </div>
                              ...
                              {/* Edit Button with dynamic routing */}
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                    ...
                                    <table className="w-full border-collapse">
                                      ...
                                      <tbody>
                                        {order.products
                                          .filter((product) => {
                                            if (viewMode === "goodsreturned") {
                                              // Only show products that have job work entries
                                              return hasJobWorkEntries(
                                                product.id,
                                                order,
                                              );
                                            }
                                            return true; // Show all products in job work view
                                          })
                                          .map((product, idx) => {
                                            if (viewMode === "jobwork") {
                                              const remainingQty =
                                              ...
                                              return (
                                                ...
                                                >
                                                  ...
                                                  <td className="border border-gray-300 px-[1.25vw] py-[0.75vw] text-[0.85vw] font-semibold">
                                                    {product.quantity || "-"}
                                                  </td>
                                                  <td className="border border-gray-300 px-[1.25vw] py-[0.75vw] text-[0.85vw] font-semibold text-orange-600">
                                                    {remainingQty}
                                                  </td>
                                                </tr>
                                              );
                                              ...
                                              return (
                                                ...
                                                >
                                                  ...
                                                  <td className="border border-gray-300 px-[1.25vw] py-[0.75vw] text-[0.85vw] font-semibold text-green-600">
                                                    {receivedQuantity}
                                                  </td>
                                                  <td className="border border-gray-300 px-[1.25vw] py-[0.75vw] text-[0.85vw] font-semibold text-orange-600">
                                                    {pendingQuantity}
