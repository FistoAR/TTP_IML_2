...
const LabelQuantitySheet = () => {
  ...
  // - productId: the unique product id (passed from Update button to resolve duplicates)
  const {
    orderId,
    companyName: incomingCompany,
    productCategory: preCategory,
    size: preSize,
    productId: preProductId, // ← NEW: unique product id from Update button
  } = location.state || {};
  ...
  // When there are duplicate products (same name+size), user picks by productId
  // preProductId locks us directly to that product; if null, user sees a picker
  const [selectedProductId, setSelectedProductId] = useState(preProductId || "");

  // Derived product (based on dropdowns + productId disambiguation)
  ...
  // All products matching the currently selected category + size
  // There may be MORE THAN ONE if duplicates exist (same name+size, different productId)
  const matchingProducts = useMemo(() => {
    if (!selectedCategory || !selectedSize) return [];
    return purchaseProducts.filter(
      (p) => p.productName === selectedCategory && p.size === selectedSize,
    );
  }, [purchaseProducts, selectedCategory, selectedSize]);

  const hasDuplicates = matchingProducts.length > 1;

  // Resolve current product:
  // 1. If a specific productId is known (from Update button or user picker) → use it
  // 2. If only one match → use it directly
  // 3. If multiple matches and no productId chosen → null (user must pick)
  useEffect(() => {
    if (!order) return;

    if (selectedCategory && selectedSize) {
      if (selectedProductId) {
        // Resolve by exact productId — handles duplicates correctly
        ...
        setCurrentProduct(matchingProducts[0]);
      } else {
        // Multiple matches, user hasn't picked one yet
        setCurrentProduct(null);
      }
    } else {
      setCurrentProduct(null);
    }
  }, [order, purchaseProducts, selectedCategory, selectedSize, selectedProductId, matchingProducts]);
    ...
    if (currentProduct) {
      ...
      if (prodRec) {
        if (currentProduct.imlType === "LID & TUB") {
          ...
          setIsSavedComplete(Boolean(prodRec.productComplete));
  ...
  const handleSave = (finalizeProduct = false) => {
    ...
    const raw = localStorage.getItem(STORAGE_KEY_LABEL_QTY);
    const all = raw ? JSON.parse(raw) : {};
    // Key is always orderId_productId — unique per product even for duplicates
    const prodKey = `${orderId}_${currentProduct.id}`;
    ...
    if (productComplete || finalizeProduct) {
  ...
  useEffect(() => {
    ...
    // Pre-select productId if passed (handles duplicates)
    if (preProductId) {
      setSelectedProductId(preProductId);
    }
  }, [preCategory, preSize, preProductId, categories]);
    ...
    <div className="min-h-screen bg-gray-50 p-[1vw]">
      <div className="max-w-[95vw] mx-auto bg-white rounded-[0.8vw] shadow-sm">
        ...
        <div className="px-[1.5vw] py-[1vw] max-h-[80vh] overflow-auto">
          ...
          {/* Product selection (category + size + duplicate resolver) */}
              ...
              <div className="grid grid-cols-2 gap-[1.2vw]">
                <div>
                  ...
                  <select
                    ...
                    className="w-full text-[.85vw] px-[0.75vw] py-[0.45vw] border border-gray-300 bg-white rounded-[0.5vw] outline-none focus:ring-2 focus:ring-purple-500 cursor-not-allowed"
                  >
              ...
              {!currentProduct ? (
                <div className="bg-gray-50 rounded-[0.6vw] border-2 border-dashed border-gray-300 p-[3vw] text-center">
                  ...
                  <p className="text-[1vw] text-gray-600 font-medium">
                    {hasDuplicates && selectedCategory && selectedSize
                      ? "Select the specific product variant above"
                ...
                <>
                  ...
                  <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-[0.6vw] border-2 border-green-200 p-[1vw] h-full">
                    ...
                    {isSavedComplete ? (
                      // All labels received - no more entry needed
                      <div className="flex items-center gap-[0.75vw] p-[1vw] bg-green-50 border-2 border-green-300 rounded-[0.5vw]">
                        <span className="text-[1.2vw]">✅</span>
                        <div>
                          <p className="text-[.9vw] font-semibold text-green-800">All labels received for this product</p>
                          <p className="text-[.8vw] text-green-600">No further entries needed. View history below.</p>
                        </div>
                      </div>
                    ) : (
                      <>
                        ...
                        <div className="mt-[1vw] grid grid-cols-2 gap-[1vw]">
                          <div className="flex items-center gap-[0.75vw] p-[0.75vw] bg-white rounded-[0.5vw] border border-gray-300">
                            ...
                            />
                            <label htmlFor="productComplete" className="text-[.85vw] font-medium text-gray-700 cursor-pointer">
