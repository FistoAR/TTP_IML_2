...
const PurchaseManagement = () => {
  ...
  const renderLabelSheet = () => {
    ...
    // Helper: get label quantity data for a product from localStorage
    const getLabelQtyData = (orderId, productId) => {
    ...
    // Helper: get TOTAL received qty by summing all history entries
                              ...
                              <div className="overflow-x-auto rounded-lg">
                                <table className="w-full border-collapse">
                                  ...
                                  <tbody>
                                    ...
                                    {purchaseProducts.map((product, idx) => {
                                      const isLidAndTub = product.imlType === "LID & TUB";
                                      const labelData = getLabelQtyData(order.id, product.id);
                                      const { lidTotal, tubTotal, singleTotal, productComplete: isProductComplete } = getTotalReceivedQty(order.id, product.id);

                                      

                                      // Order Qty cell content
                                      const orderQtyCell = isLidAndTub ? (
                                        <div className="flex flex-col gap-[0.2vw]">
                                          <span className="text-blue-700 font-semibold">LID: {product.lidLabelQty || 0}</span>
                                          <span className="text-orange-600 font-semibold">TUB: {product.tubLabelQty || 0}</span>
                                        </div>
                                      ) : (
                                        <span>{product.imlType.includes("LID") ? product.lidLabelQty : product.tubLabelQty || "0"}</span>
                                      );

                                      // Received Qty cell content
                                      const receivedQtyCell = isLidAndTub ? (
                                        <div className="flex flex-col gap-[0.2vw]">
                                          <span className="text-blue-700 font-semibold">LID: {lidTotal}</span>
                                          <span className="text-orange-600 font-semibold">TUB: {tubTotal}</span>
                                        </div>
                                      ) : (
