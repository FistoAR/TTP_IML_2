...
const PurchaseManagement = () => {
  ...
  // Group by product category → size → company (for Label sheet)
  const groupByProductCategory = () => {
    ...
    purchaseOrders.forEach((order) => {
      ...
      order.products
        ?.filter((p) => p.moveToPurchase)
        .forEach((product) => {
          const category = product.productName;
  ...
  // Open label quantity sheet (category-level, no specific product)
  ...
  // Helper: get PO details for a product from localStorage
                              ...
                              <table className="w-full border-collapse">
                                ...
                                <tbody>
                                  {purchaseProducts.map((product, idx) => {
                                    ...
                                    // === Label Order Qty cell ===
                                    let labelOrderQtyCell;
                                    if (isLidAndTub) {
                                      labelOrderQtyCell = (
                                        <div className="flex flex-col gap-[0.2vw]">
                                          <span className="text-blue-700 font-semibold">
                                            LID: {product.lidLabelQty || 0}
                                          </span>
                                          <span className="text-orange-600 font-semibold">
                                            TUB: {product.tubLabelQty || 0}
                                          </span>
                                        </div>
                                      );
                                    } else {
                                      const quantity =
                                        product.imlType.includes("LID")
                                          ? product.lidLabelQty
                                          : product.tubLabelQty;
                                      labelOrderQtyCell = (
  ...
  // FIX: Pass productId so LabelQuantitySheet can correctly resolve the exact product
  // even when multiple products share the same productName + size (duplicates).
  const handleOpenLabelQuantitySheetForProduct = (order, product) => {
    navigate("/iml/purchase/label-quantity-sheet", {
      state: {
        ...
        size: product.size,
        productId: product.id, // ← KEY FIX: pass the unique product id
      },
    });
  };

  // Label Sheet Rendering
  const renderLabelSheet = () => {
    ...
    // Read total received quantities by orderId + productId (keyed by unique productId, always correct)
    const getTotalReceivedQty = (orderId, productId) => {
      ...
      const lidTotal = history.reduce(
        (sum, h) => sum + (h.lidReceivedQuantity ?? 0),
        0
      );
      const tubTotal = history.reduce(
        (sum, h) => sum + (h.tubReceivedQuantity ?? 0),
        0
      );
      const singleTotal = history.reduce(
        (sum, h) => sum + (h.receivedQuantity ?? 0),
        0
      );
                                ...
                                <table className="w-full border-collapse">
                                  ...
                                  <tbody>
                                    {purchaseProducts.map((product, idx) => {
                                      ...
                                      // Label Order Qty cell
                                      const orderQtyCell = isLidAndTub ? (
                                        <div className="flex flex-col gap-[0.2vw]">
                                          <span className="text-blue-700 font-semibold">
                                            LID: {product.lidLabelQty || 0}
                                          </span>
                                        ...
                                        <span>
                                          {product.imlType.includes("LID")
                                            ? product.lidLabelQty
                                            : product.tubLabelQty || "0"}
                                        </span>
                                      );

                                      // FIX: Read received qty keyed by unique productId — each duplicate
                                      // product has its own storage key so totals are always per-product.
                                      const {
                                        lidTotal,
                                        tubTotal,
                                        singleTotal,
                                        productComplete: isProductComplete,
                                      } = getTotalReceivedQty(
                                        order.id,
                                        product.id
                                      );

                                      // Received Qty cell content
                                      const receivedQtyCell = isLidAndTub ? (
                                        <div className="flex flex-col gap-[0.2vw]">
                                          <span className="text-blue-700 font-semibold">
                                            LID: {lidTotal}
                                          </span>
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          <td className="border border-gray-300 px-[1vw] py-[.6vw] text-center">
                                            {isProductComplete ? (
                                              <span className="inline-flex items-center gap-1 text-black rounded-[0.4vw] text-[.85vw] font-semibold">
                                                ✅ All Labels Received
                                              </span>
                                            ) : (
                                              <button
                                                onClick={() =>
                                                  handleOpenLabelQuantitySheetForProduct(
                                                    order,
                                                    product
                                                  )
                                                }
