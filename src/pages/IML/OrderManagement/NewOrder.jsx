...
export default function NewOrder({
  ...
  const PRODUCT_SIZE_OPTIONS = {
    Round: ["120ml", "200ml", "250ml", "300ml", "500ml", "750ml", "1000ml"],
    "Glass Round": ["250ml"],
  ...
  const getAutoImlType = (productName, size) => {
    return IML_TYPE_MAP[productName]?.[size] || "";
  };
  ...
  const [products, setProducts] = useState([
    {
      ...
      isCollapsed: false,
      // NEW: Track linked design source
      linkedDesignSource: null, // Stores { productId: id, productIndex: number }
      useLinkedDesign: false, // Whether to use linked design
    },
  ]);
  ...
  // Replace the existing order initialization useEffect
useEffect(() => {
    if (existingOrder) {
      setContact(existingOrder.contact);
      
      // ===== FIXED: Don't JSON serialize - it destroys File objects =====
      // Deep clone without destroying File objects
      const clonedProducts = (existingOrder.products || []).map(product => {
        const cloned = { ...product };
        // File objects are preserved by reference (not deep cloned)
        // This is intentional - File objects can't be serialized
        return cloned;
      });
    ...
    setProducts(prevProducts => {
      const newProducts = prevProducts.map(p => {
        if (p.id === id) {
          ...
          // ✅ DEBUG
          console.log(`[updateProductWithDesignStatus] Product ${id}:`);
          ...
          console.log(`  designStatus:`, merged.designStatus);
  ...
  // UPDATED: Validate duplicate products - NOW ALLOWS SAME COMBINATION
  const isDuplicateProduct = (productName, size, imlType, currentId) => {
    console.log(
      `Checking duplicate for productName: ${productName}, size: ${size}, imlType: ${imlType}`,
    );
    // Return false to always allow selection - no duplicate blocking
    return false;
  };
...
const validateForm = useCallback(() => {
    ...
    const invalidProducts = products.filter((product, idx) => {
      ...
      if (product.useLinkedDesign) {
        hasDesign = true;
      } 
      else if (product.designSharedMail || product.designStatus === "pending") {
        hasDesign = true;
      }
      else if (product.designStatus === "approved") {
        if (product.imlType === "LID & TUB") {
          const hasLidDesign = !!product.lidDesignFile || !!product.lidSelectedOldDesign;
          const hasTubDesign = !!product.tubDesignFile || !!product.tubSelectedOldDesign;
          ...
          hasDesign = !!product.lidDesignFile || !!product.lidSelectedOldDesign;
        } else if (product.imlType === "TUB") {
          // ✅ FIX: For single TUB type, new design upload stores in lidDesignFile
          // Check BOTH lidDesignFile and tubDesignFile to cover all cases
          hasDesign = !!product.lidDesignFile || !!product.tubDesignFile || !!product.lidSelectedOldDesign || !!product.tubSelectedOldDesign;
        }
      }
    ...
    if (invalidProducts.length > 0) {
      const productErrors = invalidProducts.map((product) => {
        ...
        if (!product.useLinkedDesign && !product.designSharedMail && product.designStatus === "approved") {
          ...
          if (product.imlType === "LID & TUB") {
            ...
            if (product.singleImlDesign) {
              ...
              hasDesign = hasLidDesign && hasTubDesign;
              if (!hasDesign) {
                if (!hasLidDesign && !hasTubDesign) {
                  designMessage = "Design for both LID & TUB required";
                } else if (!hasLidDesign) {
                  designMessage = "LID Design required";
                } else {
                  designMessage = "TUB Design required";
                }
              }
            }
          } else if (product.imlType === "LID") {
            ...
            // ✅ FIX: Same fix here - check both fields
            hasDesign = !!product.lidDesignFile || !!product.tubDesignFile || !!product.lidSelectedOldDesign || !!product.tubSelectedOldDesign;
          }
          
          if (!hasDesign) missingFields.push(designMessage);
        }
        ...
        if (product.imlType === "LID & TUB") {
          if (
            (product.lidLabelQty?.trim() && !product.lidProductionQty?.trim()) ||
                ...
                {!product.isCollapsed && (
                  <>
                      ...
                      <div className="grid grid-cols-4 gap-[1.5vw]">
                        ...
                        <Select
                          ...
                          onChange={(e) => {
                            ...
                            // Check for duplicates - now we allow duplicates
                        ...
                        <Select
                          ...
                          options={
                          (() => {
                            const type = getAutoImlType(product.productName, product.size);
                            console.log(`Type: ${type}`);
                            if (type === "LID & TUB") return ["LID", "TUB", "LID & TUB"];
                            if (type === "LID") return ["LID"];
                            if (type === "TUB") return ["TUB"];
                            return ["Select Product Category & Size"]; // placeholder if none matches
                          })()
                        }
                        ...
                        >
                          <label className="block text-[0.85vw] font-medium text-gray-700 mb-[0.5vw]">
                            IML Name <span className="text-red-500">*</span>
                          </label>
                          <input
                            type="text"
                    ...
                    <div className="bg-gradient-to-br from-purple-50 to-blue-50 rounded-[0.6vw] border-2 border-purple-200 p-[1vw] mb-[0] relative">
                      ...
                      {(product.imlType === "LID & TUB" && !product.designSharedMail) && (
                      ...
                      {product.useLinkedDesign && product.linkedDesignSource && (
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-[0.6vw]">
                              ...
                              <span className="text-[0.9vw] font-medium text-green-800">
                                Using design from Product #{product.linkedDesignSource.productIndex}
                              </span>
                            </div>
                            <button
                              onClick={() => handleLinkedDesignToggle(product.id, false)}
                                      ...
                                      <div className="grid grid-cols-3 gap-[1.5vw]">
                                        {OLD_DESIGN_FILES.map((file) => (
                                          <div
                                            ...
                                            onClick={() =>
                                              !product.useLinkedDesign && updateProductWithDesignStatus(
                                                product.id,
                                          ...
                                          >
                                            <div
                                              className={`w-[6vw] h-[6vw] mx-auto bg-gray-100 rounded-[0.6vw] flex items-center justify-center text-[3vw] mb-[0.8vw] border-2 ${
                                                ...
                                                product.useLinkedDesign ? "cursor-not-allowed opacity-60" : "cursor-pointer hover:border-blue-400"
                                              } transition-all`}
                                            >
                                              {file.type === "pdf" ? (
                                                pdfPreviews[
                                            ...
                                            >
                                              <div
                                                className={`w-[6vw] h-[6vw] mx-auto bg-gray-100 rounded-[0.6vw] flex items-center justify-center text-[3vw] mb-[0.8vw] border-2 ${
                                                  ...
                                                  product.useLinkedDesign ? "cursor-not-allowed opacity-60" : "cursor-pointer hover:border-purple-400"
                                                } transition-all`}
                                              >
                                                {file.type === "pdf" ? (
      ...
      <div className="flex flex-col items-center justify-center h-full">
        ...
        {!file ? (
          <label
            ...
            className={`flex flex-col items-center w-full ${disabled ? "cursor-not-allowed" : "cursor-pointer"}`}
          >
            <div
            ...
            >
              {disabled ? "Design locked (linked)" : (isDragging ? "Drop file here" : "Upload Design File")}
            </p>
            <p className="text-[0.75vw] text-gray-400 my-[0.2vw]">
              {disabled ? "Unlink to modify design" : "Click to browse or drag & drop"}
            </p>
          ...
          <div className="w-full">
            {fileType && fileType?.startsWith("image/") && previewUrl ? (
              <div className="relative">
                ...
                <button
                  onClick={removeFile}
                  className={`absolute top-2 right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-600 transition-all shadow-md ${disabled ? "hidden" : ""}`}
                  title="Remove file"
                >
                  ×
                </button>
            ...
            <label
              htmlFor={`file-upload-${productId}`}
              className={`mt-3 block w-full ${disabled ? "cursor-not-allowed" : ""}`}
            >
              <div className={`text-center px-3 py-2 border border-blue-500 text-blue-600 rounded-[0.4vw] text-[0.8vw] font-medium hover:bg-blue-50 transition-all ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}>
                {disabled ? "Design Locked" : "Change File"}
              </div>
            </label>
          </div>
        )}
