...
const InventoryDetails = () => {
  ...
  const loadAvailableProducts = (order) => {
    try {
      ...
      const storedVerified = localStorage.getItem(STORAGE_KEY_INVENTORY_FOLLOWUPS);
      const allVerified = storedVerified ? JSON.parse(storedVerified) : {};
      const orderVerified = allVerified[order.id] || [];

      // Convert products object to array
      const productsArray = Object.values(order.products).map((product, idx) => ({
        ...product,
        index: idx + 1,
      }));
  ...
  const handleVerifyAndSend = () => {
    ...
    try {
      ...
      selected.forEach(([productId, data]) => {
        ...
        const existingIndex = allVerified[order.id].findIndex(
        ...
        const verifiedEntry = {
          ...
          finalQty: data.alreadyVerified + data.quantity,
          ...
          createdAt: existingIndex >= 0
            ? allVerified[order.id][existingIndex].createdAt
            : new Date().toISOString(),
          updatedAt: new Date().toISOString(),
        };

        if (existingIndex >= 0) {
          allVerified[order.id][existingIndex] = verifiedEntry;
        } else {
          allVerified[order.id].push(verifiedEntry);
        }
      });
                    ...
                    <tbody>
                      {verifiedData.map((entry, idx) => {
                        const isFullyVerified =
                          entry.finalQty === entry.productionQty;
                        return (
                          <tr key={idx} className="hover:bg-green-50">
                            ...
                            <td className="border border-green-300 px-[1vw] py-[0.6vw] text-[0.85vw] font-semibold text-gray-700">
                              {parseInt(entry.productionQty).toLocaleString("en-IN")}
                            </td>
                            <td className="border border-green-300 px-[1vw] py-[0.6vw] text-[0.85vw] font-bold text-green-700">
                              {parseInt(entry.finalQty).toLocaleString("en-IN")}
                            </td>
                            <td className="border border-green-300 px-[1vw] py-[0.6vw] text-[0.85vw]">
                              {parseInt(entry.samplesTaken).toLocaleString("en-IN")}
                            </td>
                            <td className="border border-green-300 px-[1vw] py-[0.6vw] text-[0.85vw]">
                              {entry.remarks}
                            </td>
