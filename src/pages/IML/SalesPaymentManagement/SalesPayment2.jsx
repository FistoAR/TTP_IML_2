...
useEffect(() => {
  if (allBills.length > 0) {
    ...
    const groupedOrders = groupBillsByCompany();
    ...
    Object.entries(groupedOrders).forEach(([companyName, orders]) => {
      ...
      Object.entries(orders).forEach(([orderKey, bills]) => {
        const orderKeyId = `${companyName}_${orderKey}`;
        newExpandedOrders[orderKeyId] = true;

        bills.forEach((bill) => {
          const billKey = `${companyName}_${orderKey}_${bill.billId}`;
          newExpandedBills[billKey] = true;
        });
      });
    });
  ...
  // Get all unique products from bills
  ...
  const handleSendToBilling = (bill) => {
    localStorage.setItem("editing_iml_sales_payment_bill", JSON.stringify(bill));
    navigate("/iml/sales-payment/details");
  };
    ...
    Object.entries(allGrouped).forEach(([companyName, orders]) => {
      Object.entries(orders).forEach(([orderKey, bills]) => {
        bills.forEach((bill) => {
          ...
          // Product filter - check if bill has products matching the product
          let matchesProduct = true;
          if (filterProduct !== "all") {
            matchesProduct = bill.products.some(
              (product) => product.productCategory === filterProduct
            );
          }
